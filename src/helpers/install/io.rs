...
use super::roots::{resolve_glob_pattern, resolve_host_path};
...
pub fn replace_in_file(path: &str, from: &str, to: &str) -> Result<(), Box<EvalAltResult>> {
    ...
    let resolved = resolve_host_path(path).map_err(|e| -> Box<EvalAltResult> { e.into() })?;
...
pub fn glob_list(pattern: &str) -> rhai::Array {
    let Ok(pattern) = resolve_glob_pattern(pattern) else {
